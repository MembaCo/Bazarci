...
class Giris extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->template->loadContent("giris/index", array());
    }
    ...
    public function ajax_check_login()
    {
        ...
        if($this->settings->info->login_protect) {
            ...
            $s = $this->Giris_Model->get_login_attempts($_SERVER['REMOTE_ADDR'],
                $email, (15*60));
            if($s->num_rows() > 0) {
                $s = $s->row();
                if($s->count >=5) {
        ...
        $login = $this->Giris_Model->getUserByEmail($email);
        if ($login->num_rows() == 0) {
            $login = $this->Giris_Model->getUserByUsername($email);
            if($login->num_rows() == 0) {
    ...
    public function pro($redirect="")
    {
        ...
        if($this->settings->info->secure_login) {
            ...
            // Store it
            $this->Giris_Model->updateUserToken($userid, $token);
        } else {
            if(empty($r->token)) {
                // Generate a token
                $token = rand(1,100000) . $email;
    ...
    private function login_protect($email)
    {
        if($this->settings->info->login_protect) {
            ...
            if($s->num_rows() > 0) {
                $s = $s->row();
                $this->Giris_Model->update_login_attempt($s->ID, array(
                        "count" => $s->count+1
                    )
                );
            } else {
                $this->Giris_Model->add_login_attempt(array(
                        "IP" => $_SERVER['REMOTE_ADDR'],
                        "username" => $email,
                        "count" => 1,
                        "timestamp" => time()
    ...
    public function google_login()
    {
        ...
        if ($client->getAccessToken()) {
            ...
            $user = $this->Giris_Model->get_oauth_user($provider, $oauth_id);
            if ($user->num_rows() == 0) {
                // Create
                $userid = $this->Kayit_Model->add_user(array(
                ...
                $default_groups = $this->Kullanici_Model->get_default_groups();
                foreach($default_groups->result() as $r) {
                    $this->Kullanici_Model->add_user_to_group($userid, $r->ID);
                }
            } else {
                $this->Giris_Model
    ...
    public function twitter_login_pro()
    {
            ...
            && !empty($_SESSION['oauth_token_secret']))
        {
            ...
            if ($user->num_rows() == 0) {
                ...
                // Update User
                $this->Giris_Model->update_oauth_user(
                    $access_token['oauth_token'],
                    $access_token['oauth_token_secret'],
                    $oauth_id,$provider);
            }
    ...
    public function resetpw($token,$userid)
    {
        ...
        // Check
        $user = $this->Giris_Model->getResetUser($token, $userid);
        if ($user->num_rows() == 0) {
    ...
    public function resetpw_pro($token, $userid)
    {
        ...
        $this->Giris_Model->updatePassword($userid, $password);
        $this->Giris_Model->deleteReset($token);
    ...
    public function forgotpw_pro()
    {
        ...
        $log = $this->Giris_Model->getResetLog($_SERVER['REMOTE_ADDR']);
        if ($log->num_rows() > 0) {
            $log = $log->row();
        ...
        $this->Giris_Model->addToResetLog($_SERVER['REMOTE_ADDR']);

        // Check for email
        $user = $this->Giris_Model->getUserEmail($email);
        if ($user->num_rows() == 0) {
            $this->template->error(
                lang("error_47")
            );
        ...
        $this->Giris_Model->resetPW($user->ID, $token);
