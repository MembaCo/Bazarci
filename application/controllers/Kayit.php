...
class Kayit extends CI_Controller
{
    ...
    public function index()
    {
            ...
            if (empty($fail)) {
                ...
                if($this->settings->info->activate_account) {
                    ...
                    $email_template->message = $this->common->replace_keywords(array(
                        ...
                        "[SITE_URL]" => site_url(),
                        "[EMAIL_LINK]" =>
                            site_url("kayit/activate_account/" . $activate_code .
                                "/" . $username),
        ...
        $this->template->loadContent("kayit\index", array(
                "cap" => $cap,
                "email" => $email,
                "first_name" => $first_name,
                "last_name" => $last_name,
    ...
    public function add_username()
    {
        ...
        $this->template->loadContent("kayit/ekle", array());
    }
    ...
    public function activate_account($code, $username)
    {
        ...
        $code = $this->Kullanici_Model->get_verify_user($code, $username);
        if($code->num_rows() == 0) {
        ...
        $this->Kullanici_Model->update_user($code->ID, array(
                "active" => 1,
                "activate_code" => ""
            )
        );
    ...
    public function send_activation_code($userid, $email)
    {
        ...
        // Check request
        $request = $this->Kullanici_Model->get_user_event("email_activation_request");
        ...
        $this->Kullanici_Model->add_user_event(array(
        ...
        // Resend
        $user = $this->Kullanici_Model->get_user_by_id($userid);
        if($user->num_rows() == 0) {
